...
describe('ASVAB quiz timer stability', () => {
    it('uses useMemo for ASVAB choices so answers are stable across re-renders', () => {
        ...
        // look for a useMemo that depends on currentAsvabQuestion so choices stay stable
        const re = /useMemo\s*\(\s*\(\)\s*=>\s*\{[\s\S]*?currentAsvabQuestion[\s\S]*?choices[\s\S]*?correctText[\s\S]*?\},\s*\[\s*currentAsvabQuestion\s*\]\s*\)/m;
